...
class TechMasterApp:
    def __init__(self, root):
        ...
        # Variables para almacenar info estática
        ...
        self.tab_red = ttk.Frame(self.notebook) # NUEVA PESTAÑA
        self.tab_opt = ttk.Frame(self.notebook) # NUEVO: Pestaña de Optimización
        ...
        if respuesta:
            def carga_cpu():
                ...
                while time.time() < t_end:
                    pass # Bucle infinito temporal para estresar el núcleo
            
            # Lanzamos un hilo por cada núcleo lógico de la CPU
    ...
    def update_monitor(self):
        ...
        # NUEVO: Cálculo del Tiempo Encendido Real (Uptime)
        uptime_secs = time.time() - psutil.boot_time()
        horas = int(uptime_secs // 3600)
        minutos = int((uptime_secs % 3600) // 60)
        
        # Actualizamos la cabecera para mostrar el uptime
        try:
...
#red y conectividad :v
    ...
    # NUEVO: Función para test de ping en hilo separado
    def test_ping(self):
        self.red_text.config(state=tk.NORMAL)
...
# cosa que no recuerdo XD
    ...
    def audit_system_errors(self, wmi_client):
        ...
        # NUEVO: Comprobar Pantallazos Azules (Minidumps)
    ...
    def build_pruebas_tab(self):
        # Contenedor superior (Teclado y Mouse)
        frame_top = tk.Frame(self.tab_pruebas)
        # Cambio: Hacemos que este marco se expanda también verticalmente
        ...
        # Cambio: Quitamos expand=True y usamos anchor="n" para que no se estire y se quede pequeño arriba
        ...
        # Cambio: Acoplamos a la izquierda pero le damos todo el permiso para expandirse (expand=True)
    ...
    def test_audio(self):
        if 'winsound' in globals():
            # Genera un pitido característico del sistema Windows
